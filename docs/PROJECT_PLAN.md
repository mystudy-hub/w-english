...
> **v2.1**：Claude Opus 评审后补充（年龄≠能力动态适配、双模式单组件架构、音频精灵工程化、向导持久化、奖励与学习解耦、指令去文字化等）
...
> ⚠️ **架构红线（Claude Opus）**：不要做两套组件/两套路由——维护面翻倍、状态不同步、SSR 水合不一致。

**正确做法：单组件 + 配置驱动（Config-Driven Single Component）**

```typescript
// 一个组件，通过 ageConfig 驱动行为差异——不是两套代码
type AgeConfig = {
  touchTargetMin: number;     // 64 | 80 (px)
  maxItemsPerGame: number;    // 3 | 5
  allowDragDrop: boolean;     // false | true
  showTextLabels: boolean;    // false | true
  screenTimeMinutes: number;  // 8 | 12 | 18
  guideAlwaysOn: boolean;     // true | false
  countdownEnabled: boolean;  // false | true
};

// ageConfig 写 cookie，server component 读 cookie 首屏即正确渲染
// 避免全站 client-side，保留 App Router SSR 收益
```
...
### 0.4 ⭐年龄 ≠ 能力：动态难度自适应（Claude Opus 关键发现）

> **核心问题**：用年龄当能力代理不成立。5 岁英语启蒙早的孩子会觉得低龄档幼稚；8 岁零基础的孩子在高龄档会持续挫败。静态按年龄分档需要被动态表现修正。
...
动态偏移 = 基于孩子实际表现自动微调
─────────────────────────────────────
实际体验 = 初始模式 + 动态偏移（上限 ±1 档）
```

- **追踪指标**（存 IndexedDB）：
  - 单词首次尝试正确率（>80% → 上探；<40% → 回调）
  - 游戏完成时间（显著快于同龄均值 → 可能是"高手"）
  - 重复点击/误触频率（高 → 可能手眼协调尚弱，不一定是认知问题）
- **切换策略**：
  - 不弹窗，无"降级"羞耻感——魔法向导自然说："这个有点难，我们先玩这个吧！"
  - 切换入口在家长面板中可见，对孩子**透明无感**。
  - 家长可锁定："我的孩子就固定在这个模式"（覆盖自动偏移）。
...
4. **正向即时反馈（Positive Reinforcement）**：
   - 杜绝惩罚性红叉或挫败音效；错误时使用温和提示音与魔法向导动画摇晃引导再试一次——"差一点点，再试一次！"而非"错了！"。正确时获得星星（Stars）、贴纸（Stickers）与掌声。
   - ⭐ **指令去文字化（Claude Opus）**：目标用户（4~7 岁）读不了文字说明。所有操作引导必须是 **图标 + 语音旁白**，不能用文字弹窗。例如"请点击正确的图片" → 魔法向导用手指向屏幕 + 语音"点点看，哪个是小猫？"。文字仅作为家长面板的可选辅助。
...
- ⭐ **向导状态持久化（Claude Opus）**：
  - 儿童会随手刷新页面、误关标签页、设备休眠唤醒。多步引导（如"先点 C → 再点 A → 再点 T"）若仅存 React state，刷新后进度全丢，孩子困惑。
  - **解决方案**：向导当前步骤、已完成的引导节点写入 IndexedDB，页面加载时先读档——若上次停在"拼读 C-A-T 的第 2 步"，刷新后从该步继续。家长面板可查看已完成引导列表。
...
> ⚠️ **奖励解耦原则（Claude Opus）**：动效和奖励过强会导致孩子为拿星星乱点——没开口、没跟读也能拿到正反馈，拼读就白做了。**奖励必须挂在发音/拼读行为上，不是挂在"点对了"上。**
>
> 具体策略：
> - 听音选图：奖励前提 = 听过发音 + 做出选择（两个条件都满足才给星）
> - 点选拼字母：奖励前提 = 依次点过所有音素气球（跳过直接看答案 = 无奖励）
> - 小怪兽喂食：奖励前提 = 听过小怪兽说的句子 + 正确投喂
> - **动态提示**：若连续 3 次只看图不听音就选对，魔法向导说"你太厉害了！要不要试试先听声音再找？"——引导而非惩罚
...
| 🆕 **动效安全 (Motion Safety)** | 闪烁/快速动画可能触发光敏感或注意力弱儿童的不适 | 提供 **reduced-motion 开关**（家长面板），开启后：禁用粒子特效、卡片展开改为淡入而非弹簧、魔法向导动画降为帧数减半的缓慢过渡。同时尊重 `prefers-reduced-motion` 系统媒体查询。 |
...
4. ⭐ **单组件驱动（Config-Driven）**：年龄模式通过配置对象注入单个组件，不就"小童/大童"各写一套组件。`ageConfig` 写 cookie，server component 读 cookie 首屏正确渲染，避免 hydration mismatch。

### 6.3 ⭐音频工程专项设计（Claude Opus 关键发现）

> **核心风险**：自然拼读依赖数百个音素/单词音频文件——请求风暴、iOS Safari AudioContext 锁定、离线不可用，任何一环断裂则核心体验崩溃。

| 风险点 | 影响 | 工程对策 |
|:---|:---|:---|
| **请求风暴** | 一个单词的音素拆分（如 apple = 5 个文件）× 30 词/场景 = 150 个 HTTP 请求，首屏白屏 | **Audio Sprite**：将同一主题的所有音素打包为单个 MP3 文件 + JSON 时间轴（`{phoneme: "æ", start: 0.5, end: 1.2}`），Howler.js sprite 模式播放片段 |
| **iOS Safari 锁** | 首次 AudioContext 必须由用户手势（touchstart/touchend）创建，自动播放静默失败 | 首页加一个"点我开始冒险！"大按钮（本质是 AudioContext.resume() 触发器），之后全局音频可用。按钮本身就是魔法向导的欢迎语触发器，体验无缝 |
| **离线不可用** | Edge-TTS 合成音频必须联网，飞机/汽车上发音失效 | 音频文件（含 sprite 合并包）全部预置在 `/public/audio/`，随 SSG 静态打包，Service Worker 缓存。Edge-TTS 仅在「构建时」使用，运行时零请求 |
| **音量突变** | 切换音素时多个 Howler 实例叠加、或精灵 seek 不准导致爆音 | Howler.js 全局单例管理，fadeIn/fadeOut（50ms 淡入淡出）消除 click 噪声；精灵时间轴允许 ±5ms 容差 |

**音频精灵打包脚本（构建时）**：
```bash
# 伪代码：将 /audio/raw/*.mp3 合并为 sprite 包
audio-sprite-builder \
  --input /audio/raw/phonics/ \
  --output /public/audio/sprites/phonics_sprite.mp3 \
  --manifest /public/audio/sprites/phonics_sprite.json
# → JSON: { "a_short": [0, 1.2], "p": [1.2, 1.8], ... }
```
...
| — | 🔴 新增 v2.1 | 动态难度自适应（年龄≠能力，追踪表现自动微调，无降级羞耻感） |
| — | 🔴 新增 v2.1 | 双模式单组件架构红线（Config-Driven，拒绝两套代码；cookie 驱动 SSR 首屏） |
| — | 🔴 新增 v2.1 | 音频精灵工程化（Audio Sprite 合并包、iOS AudioContext 解锁、离线优先、构建时打包） |
| — | 🔴 新增 v2.1 | 向导状态 IndexedDB 持久化（刷新恢复、断点续接） |
| — | 🔴 新增 v2.1 | 奖励解耦原则（奖励挂在发音/拼读行为上，而非"点对了就送星"） |
| — | 🔴 新增 v2.1 | 指令去文字化（4-7 岁不认字，全部用图标+魔法向导语音替代文字弹窗） |
| — | 🔴 新增 v2.1 | 动效安全开关（reduced-motion，防光敏感/注意力弱儿童不适） |
