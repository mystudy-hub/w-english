...
export function WordCard({ wordId }: { wordId: string }) {
  ...
  const play = async (ref: string, countHeard: boolean) => {
    ...
    setPlaying((current) => current === ref ? undefined : current);
  ...
  return <><Header back={back}><span className="header-chip"><BookOpen size={16} /> 认识一个新朋友</span></Header><main className={`word-page ${parentOpen ? 'with-parent' : ''}`}>
    <section className="word-card-surface"><div className="word-portrait"><span className="portrait-circle" /><WordImage word={word} /><span className="portrait-spark">✧</span></div>
      <div className="word-details"><span className="word-eyebrow">HELLO, LITTLE FRIEND</span><h1>{word.word}</h1><p className="word-definition">{word.definition[LEVELS[settings.learningLevel].definition]}</p>
        <div className="audio-controls"><Button className="primary" disabled={!hasAudio} onClick={() => void play(word.audio.word, true)} aria-label="听单词">{playing === word.audio.word ? <LoaderCircle className="spinning" /> : <Volume2 />}<span data-mode-label>听一听</span></Button><Button className="soft-gold" disabled={!audio.has(word.audio.wordSlow)} onClick={() => void play(word.audio.wordSlow, true)} aria-label="慢慢听"><Turtle /><span data-mode-label>慢慢听</span></Button><Button className="soft-blue" disabled={!audio.has(word.example.audio)} onClick={() => void play(word.example.audio, false)} aria-label="听例句"><Headphones /><span data-mode-label>听句子</span></Button></div>
